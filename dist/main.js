...
async function run() {
    ...
    const release = await findVersion(gitHubApi, (0, core_1.getInput)("version"));
    const asset = findAsset(release.assets, GITHUB_RELEASE_ASSET_NAME);
    const restorePath = path.normalize((0, core_1.getInput)("install-path"));
    ...
    const executablePath = path.normalize((0, core_1.getInput)("path"));
    let phpStanBin;
    try {
        await copyExecutable(executablePath, restorePath);
        (0, core_1.info)(`${ACTION_OUT_PREFIX} Using provided phpstan executable '${executablePath}'`);
    }
    catch (err) {
        if (executablePath != "undefined") {
            (0, core_1.info)(`${ACTION_OUT_PREFIX} Provided executable could not be found, falling back to target version ${release.tag_name} released @ ${release.published_at}`);
        }
        else {
    ...
    (0, core_1.setOutput)("phpstan", phpStanBin);
    (0, core_1.addPath)(path.dirname(phpStanBin));
}
exports.run = run;
;
(async () => {
